...
### 게시글 좋아요 동시성 이슈
- AtomicLong 타입을 이용하여 좋아요 연산을 낙관적 Lock으로 구현하려 했으나 DB에 바이트 코드 형태로 좋아요 숫자가 들어가 DB의 값을 육안으로 보기 어려웠습니다.
- update 쿼리를 실행하기 전 조회 쿼리를 Shared Lock(Pessimistic Lock) 으로 로직을 변경 하였습니다. ( 읽기 작업이 많기 때문에 데드락을 대비하기 위해 좋아요 집계 테이블을 정규화 하고 Shared Lock으로 대체)
