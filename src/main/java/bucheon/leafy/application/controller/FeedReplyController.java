...
public class FeedReplyController {
    ...
    @GetMapping
    public ResponseEntity<List<FeedReplyResponse>> getReplies(@PathVariable Long feedId,
    ...
    public ResponseEntity<Long> saveReply(@AuthenticationPrincipal @Parameter(hidden = true) AuthUser user,
                                          @PathVariable Long feedId,
                                          @PathVariable Long commentId,
                                          @RequestBody FeedReplyRequest request) {
    ...
    @PutMapping("/{replyId}")
    @PreAuthorize("hasAnyRole('MEMBER', 'ADMIN')")
    public ResponseEntity<Map<String, Object>> updateReply(@AuthenticationPrincipal @Parameter(hidden = true) AuthUser user,
                                                           @PathVariable Long feedId,
                                                           @PathVariable Long commentId,
    ...
    @PreAuthorize("hasAnyRole('MEMBER', 'ADMIN')")
    public ResponseEntity deleteReply(@AuthenticationPrincipal @Parameter(hidden = true) AuthUser user,
                                      @PathVariable Long feedId,
                                      @PathVariable Long commentId,
                                      @PathVariable Long replyId) {
        Long userId = user.getUserId();
